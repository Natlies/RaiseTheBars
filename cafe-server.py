...
if os.name == 'nt':
    os.system("color")
    os.system("title RaiseTheBars Server")
else:
    sys.stdout.write("\x1b]2;RaiseTheBars Server\x07")
...
@app.route("/play2.html", methods=['GET'])
def play2():
    if 'UID' not in session:
        return redirect("/")
    ...
    return render_template("play2.html",
                           ...
                           # swf_file="Game.74708-tracer2a.swf", # has fixes
                           # swf_file="Game.74708-tracer2b.swf",  # has fixes, lightweight
                           swf_file="Game.a35e0ccaa60f417447aeed760afb511b6e675e17.lzma.swf",  # 2b with debugging symbols
                           # swf_file="Game.74708-tracer2x.swf",  # add traced files on demand
                           # swf_file="Game.74708-tracer2y.swf",  # add traced files on demand
                           version=version_name,
                           release_date=release_date,
...
@app.route("/127.0.0.1flashservices/gateway.php", methods=['POST'])
def flashservices_gateway():
    ...
    for reqq in reqs:
        ...
        UID = resp_msg.bodies[0][1].body[0].get("zid","a050a4c0-379b-4050-850e-131eacf71494")  #temporary hardcoded
    ...
    emsg = {
        ...
        "errorType": 0,
        "data": resps,
        "clientRuntimeValues": {"qualifiedListsOverrides": {
            "QUALIFIED_LIST_OVERRIDE_RESTAURANT_SUPPLIES_ADD": "",
            "QUALIFIED_LIST_OVERRIDE_RESTAURANT_SUPPLIES_REMOVE": "",
        } }
    }
...
@app.route("/assets/SharedFonts_en_US__dfd2c.swf")
@app.route("/assets/hashed/SharedFonts_en_US__dfd2c.swf")
def fonts():
    return send_from_directory("assets", "SharedFonts_en_US__7b323.swf")


@app.route("/assets/content__a19bb.amf")
@app.route("/assets/hashed/content__a19bb.amf")
def compressed_content():
    return send_from_directory("assets", "content__a19bb_compressed.amf")
...
@app.route("/ThePreloader.swf")  # Game.a35e0ccaa60f417447aeed760afb511b6e675e17.lzma.swf
def flashFile():
   ## return send_from_directory("assets", "Game.a35e0ccaa60f417447aeed760afb511b6e675e17.lzma.swf")  # regular one
    return send_from_directory("assets", "Preloader.lzma.swf")  # regular one@app.route("/Game.a35e0ccaa60f417447aeed760afb511b6e675e17.lzma.swf")

@app.route("/assets/Game.a35e0ccaa60f417447aeed760afb511b6e675e17.lzma.lzma.swf")
def flashFile2():
   #return send_from_directory("assets", "Game.a35e0ccaa60f417447aeed760afb511b6e675e17.lzma.swf")  # regular one
   #return send_from_directory("assets", "Game.a35e0ccaa60f417447aeed760afb511b6e675e17_tracer1a.lzma.swf")  # with logging one
   #return send_from_directory("assets", "Game.a35e0ccaa60f417447aeed760afb511b6e675e17_tracer1b.lzma.swf")  # with logging one & logging during debug
   return send_from_directory("assets", "Game.a35e0ccaa60f417447aeed760afb511b6e675e17_tracer2_gutted.lzma.swf")  # gutted version without missing
   # return send_from_directory("assets", "Preloader.lzma.swf")  # regular one


@app.route("/assets/<path:path>")
@app.route("/assets/hashed/<path:path>")
def assets(path):
    #cw2_env_tuscan_floor_tile_stone__c9f01.swf
    try:
        ...
        if "cw2_" in path and ".swf" in path:
            ...
            elif "anim_" in path and "idlestand" in path:
                print(error)
                print("substituting3", path)
                return send_from_directory("assets", "cw2_anim_story_jamie_oliver_idle__76ac8.swf")
            else:
                print(error)
                print("substituting", path)
                return send_from_directory("assets", "cw2_env_tuscan_floor_tile_stone__c9f01.swf")
        else:
            raise error
...
@app.route('/127.0.0.1client_error_message.php', methods=['POST'])
def log_error():
    print("Client Error:")
    payload = request.form.get('payload')
    print(repr(payload))
